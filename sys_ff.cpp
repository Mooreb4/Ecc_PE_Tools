...
// g++ -I/Users/blakemoore/eigen -I/Users/blakemoore/fftw/include -I/Users/blakemoore/Desktop/Grad\ Research/MCMC_ecc/MODEL2 -I/Users/blakemoore/gsl/include -O3 -ffast-math -g3 -c -fmessage-length=0 -std=c++14 -MMD -MP -MF"sys_ff.d" -MT"sys_ff.o" -o "sys_ff.o" "sys_ff.cpp"
//build with
// g++ -L/Users/blakemoore/gsl/lib -L/Users/blakemoore/fftw/lib -o "tst_sys" ../TaylorT4.o ../Amps.o ../TaylorF2e.o ../llhood_maxd.o ../fisher.o ./sys_ff.o  -lfftw3 -lgsl -lgslcblas -lm

using namespace std;
using namespace Eigen;

string path_noise = "/mnt/lustrefs/work/blake.moore2/MCMC_ecc/noise/";
string path_samples = "/mnt/lustrefs/work/blake.moore2/MCMC_ecc_Model3/systematics_corr/results/";
string path_chains = "/mnt/lustrefs/work/blake.moore2/MCMC_ecc_Model2/systematics_corr/chains/";
string path_likelihoods = "/mnt/lustrefs/work/blake.moore2/MCMC_ecc_Model2/systematics_corr/likelihoods/";
...
//string path_samples = "/Users/blakemoore/Desktop/Grad Research/MCMC_ecc/MODEL2/systematics/samples_DB/";
//string path_chains = "/Users/blakemoore/Desktop/Grad Research/MCMC_ecc/MODEL2/systematics/chains/";
//string path_likelihoods = "/Users/blakemoore/Desktop/Grad Research/MCMC_ecc/MODEL2/systematics/likelihoods/";
...
vector<double> gen_p0_e0(double M, double eta, double e0, double A, double f0, double fend, double df);
...
int main(int argc, const char * argv[]){
    ...
    ////////////////////////////////////////////////////////
    // Now sort out the TaylorT4 type injection
    // resample noises in case df has naturally changed
    ////////////////////////////////////////////////////////
    
    vector<double> ep = gen_p0_e0(M_in, eta_in, e0_in, A_2, 0, fend, df);
    
    double D_l = .211772/A_2;
    double M_tot = M_in/pow(eta_in, 3./5.);
    TaylorT4 t4(ep[1], ep[0], M_tot, eta_in, D_l, 3./7., 3./7., 3./7., 3./7., 3./7., 8192, 3);
    vector<vector<double> > vect;
    if(stoi(argv[2]) == 1){
        vect = t4.get_hfsol_downsamp(4096, 0.015);}
    else{
        vect = t4.get_hfsol_downsamp_e(4096, 0.015);
    }
    double e_ref_injection = t4.get_eref();
    int newsize = vect.size();
    
    h2.resize(newsize);
    for(int i = 0; i < newsize; i++){
        h2[i] = vect[i][1] + 1i*vect[i][2];
    }
    freqs.resize(newsize);
    for(int i = 0; i < newsize; i++){
        freqs[i] = vect[i][0];
    }
    fend = vect[newsize - 1][0];
    df = vect[1][0];
    
    //noise for the likelihood
    noise.resize(newsize);
    for (int i = 0; i < newsize; i++)    {
        if (freqs[i] > 1 &&  freqs[i] < 4096 ) {
            noise[i] = gsl_spline_eval (spline, freqs[i], acc);
        } else {
    ...
    //downsampled noise for the fisher
    N_down_noise = fend/df_fish + 1;
    noise_fish.resize(N_down_noise);
    f = 0;
    for (int i = 0; i < N_down_noise; i++)    {
        f = df_fish*i;
        if (f > 1 &&  f < 4096 ) {
    ...
    cout << "Injections all finished up" << endl;
    ...
    for(int i = 0; i < N_chain; i++){
        chain c(M_in, eta_in, e_ref_injection, A_2, 1*pow(spacing, i), fend, df, df_fish, ep_fish, noise, noise_fish, h2);
        chains.push_back(c);
    }
    
    double curr_max_loglike = chains[0].loglike_loc;
    double max_Mc = M_in;
    double max_eta = eta_in;
    double max_e = e_ref_injection;
    double max_A = A_2;
    double max_FF = match(max_Mc, max_eta, max_e, max_A, 0, fend, df, h2, noise);
    
    vector<double> max_dat(6);
    max_dat[0] = max_Mc;
    max_dat[1] = max_eta;
    max_dat[2] = max_e;
    max_dat[3] = max_A;
    max_dat[4] = max_FF;
    max_dat[5] = e_ref_injection;
    int count_max = 0;
    
    cout << "chains initialized" << endl;
...
//        }
        ...
        //        if ( i % 20000 == 0){
        //
        //            write_vec_to_file(chain_store[0], "Samples_N_"+to_string(N_jumps)+"_chain_"+to_string(0)+"_Mc_"+argv[3]+"e_ref"+argv[5]+".txt", path_samples);
        //            write_vec_to_file(like_store[0], "likelihood_N_"+to_string(N_jumps)+"_chain_"+to_string(0)+"_Mc_"+argv[3]+"e_ref"+argv[5]+".txt", path_likelihoods);
        //
        //            for(int j = 1; j < N_chain; j++){
        //                write_vec_to_file(chain_store[j], "Samples_N_"+to_string(N_jumps)+"_chain_"+to_string(j)+"_Mc_"+argv[3]+"e_ref"+argv[5]+".txt", path_chains);
        //                write_vec_to_file(like_store[j], "likelihood_N_"+to_string(N_jumps)+"_chain_"+to_string(j)+"_Mc_"+argv[3]+"e_ref"+argv[5]+".txt", path_likelihoods);
        //            }
        //        }
        
        // Keep an update of the ML, FF, and associated parameters for the cold chain
        if(chains[0].loglike_loc > curr_max_loglike){
            curr_max_loglike = chains[0].loglike_loc;
            max_Mc = chains[0].loc[0];
            max_eta = chains[0].loc[1];
            max_e = chains[0].loc[2];
            max_A = chains[0].loc[3];
            max_FF = match(max_Mc, max_eta, max_e, max_A, 0, fend, df, h2, noise);
            max_dat[0] = max_Mc;
            max_dat[1] = max_eta;
            max_dat[2] = max_e;
            max_dat[3] = max_A;
            max_dat[4] = max_FF;
            count_max = i - count_max;
            write_vec_to_file(max_dat, "Max_val_Mc_"+to_string(stod(argv[3]))+"e_ref"+to_string(stod(argv[5]))+"+_T4_+"+to_string(stoi(argv[2]))+".txt", path_samples);
            cout << setprecision(16) << "Max Like = " << curr_max_loglike << " Max FF = " << max_FF << " e_ML = " << max_e << " e_inj = " << e_ref_injection << endl;
            cout << "Steps since last max = " << count_max << endl;
        }
    }
...
int check_priors(vector<double> &prop){
    int cont = 0;
    if(prop[0] > 15.1 || prop[0] < 0.8) {cont = 1;}
    if(prop[1] > 0.25 || prop[1] < 0.15) {cont = 1;}
    if(prop[2] > 0.81 || prop[2] < 0.000001) {cont = 1;}
    if(prop[3] > 1e-10 || prop[3] < 1e-20) {cont = 1;}
    return cont;
}
void prior_prop(vector<double> &prop, const gsl_rng * r){
    prop[0] = exp(gsl_ran_flat(r, -0.223144, 2.70805));
    prop[1] = gsl_ran_flat(r, 0.15, 0.25);
    prop[2] = gsl_ran_flat(r, 0.000001, 0.81);
    prop[3] = exp(gsl_ran_flat(r, -46.0517, -23.0259));
}
void set_loc(vector<double> &prop, vector<double> &loc){
    loc[0] = prop[0];
    loc[1] = prop[1];
...
vector<double> gen_p0_e0(double M, double eta, double e0, double A, double f0, double fend, double df){
    ...
    F2e.init_interps(8000);
    F2e.make_scheme();
    vector<double> ep = F2e.get_p0_e0_max();
    return ep;
}
vector<complex<double>> sum_f2(vector<vector<complex<double>>> &vect){
    int N = vect[0].size();
